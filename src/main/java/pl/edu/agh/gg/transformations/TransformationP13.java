...
public class TransformationP13 implements DoubleInteriorTransformation {
    ...
    public boolean isApplicable(GraphModel graph, InteriorNode firstInterior, InteriorNode secondInterior) {
        ...
        return graph.getEdgeBetweenNodes(v1_1, v2_1).isEmpty() && graph.getEdgeBetweenNodes(v1_2, v2_2).isEmpty()
                && graph.getEdgeBetweenNodes(v1_1, v1_2).isPresent() && graph.getEdgeBetweenNodes(v2_1, v2_2).isPresent();
    }
    ...
    private List<Vertex> getNodesWithTheSameCoordsFromNextLayer(InteriorNode firstInterior, InteriorNode secondInterior, GraphModel graph) {
        Vertex[] firstInteriorAdjacents = getValidAdjacentInterior(firstInterior, graph).getAdjacentVertices().toArray(new Vertex[0]);
        Vertex[] secondInteriorAdjacents = getValidAdjacentInterior(secondInterior, graph).getAdjacentVertices().toArray(new Vertex[0]);
        List<Vertex> common = new LinkedList<>();
        for (Vertex v1 : firstInteriorAdjacents) {
    ...
    private List<Vertex> getSharedNodesFromNextLayer(InteriorNode firstInterior, InteriorNode secondInterior, GraphModel graph) {
        Vertex[] firstInteriorAdjacents = getValidAdjacentInterior(firstInterior, graph).getAdjacentVertices().toArray(new Vertex[0]);
        Vertex[] secondInteriorAdjacents = getValidAdjacentInterior(secondInterior, graph).getAdjacentVertices().toArray(new Vertex[0]);
        List<Vertex> common = new LinkedList<>();
        for (Vertex v1 : firstInteriorAdjacents) {
            for (Vertex v2 : secondInteriorAdjacents) {
                if (v1.equals(v2) && v1.getUUID().equals(v2.getUUID())) common.add(v1);
            }
        }
        return common;
    }
